  ...
  # Import NcQuestions from CSV file
	def import_files
      ...
      begin
        ...
        redirect_to new_nc_question_path, :flash => { :notice => MESSAGES["nc_question"]["csv_upload"]["error"]}
      end
    else
      redirect_to new_nc_question_path , :flash => { :notice => MESSAGES["nc_question"]["csv_upload"]["presence"]}
    end
  end
